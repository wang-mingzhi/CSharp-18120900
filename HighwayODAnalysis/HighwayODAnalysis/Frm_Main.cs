...
namespace HighwayODAnalysis
{
    public partial class Frm_DataAnalysis : Form
    {
        ...
        private void Tsb_OpenFolder_Click(object sender, EventArgs e)
        {
            ...
            {
                PublicVariable.SplitedFilePath.Clear();
        ...
        private void Tvw_Folder_AfterCheck(object sender, TreeViewEventArgs e)
        {
            ...
            if(!PublicVariable.SplitedFilePath.Contains(temp_path) & e.Node.Checked)
            {
            ...
            lbl_FolderInfo.Text = $"文件夹目录:({PublicVariable.SplitedFilePath.Count})";
        }
                ...
                for (int i = 0; i < Dgv_StatisticSetting.Rows.Count; i++)
                {
                    if ((bool)Dgv_StatisticSetting.Rows[i].Cells[index].EditedFormattedValue)
                    {
                        statisticSetting = new PublicVariable.StatisticSetting
                        {
                            ...
                            DataType=Dgv_StatisticSetting.Rows[i].Cells[PublicVariable.DGVStruck.DataTypeIndex].Value.ToString(),
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                StreamWriter sw = new StreamWriter(@"F:\18120900\桌面\ningxia_File.csv", false, Encoding.Default);
                sw.WriteLine("EntryStation\tEntryTime\tExitStation\tExitTime\tJudgeRodSpecification\tJudgeStatus\tMiles\tOrgAxisName\tTotalWeight\tTotalLimit\tTotalToll\tRealToll\tOrgAxisNum");
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ID = line.Split(new char[] { '\t' })[7];
                    Label:
                    if (keyValuePairs.ContainsKey(ID))
                    {
                        line += "\t" + keyValuePairs[ID];
                    }
                    else
                    {
                        keyValuePairs.Add(ID, 0);
        ...
        private void Frm_DataAnalysis_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            for (int i = 0; i < PublicVariable.SplitedFilePath.Count; i++)
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            MatrixStruce.Matrix.Add(key, new List<double>() { miles, 1 });
                        }
                    }
                    catch (Exception)
                    {
        ...
        {
            StreamWriter sw = new StreamWriter("F:\\18120900\\桌面\\2018_truck_data.txt", false, Encoding.Default);
            for (int i = 0; i < PublicVariable.SplitedFilePath.Count; i++)
            {
                Tssl_ProgressInfo.Text = i + 1 + "/" + PublicVariable.SplitedFilePath.Count;
                StreamReader sr = new StreamReader(PublicVariable.SplitedFilePath[i], Encoding.Default);
                ...
                {
                    n += 1;
                    string veh = line.Split(new char[] { '\t' }, count: 6)[4];
                    if (veh == "11" | veh == "12" | veh == "13" | veh == "14" | veh == "15") 
                    {
                        sw.WriteLine(line);
                    }
                    if (n % 10000 == 0)
